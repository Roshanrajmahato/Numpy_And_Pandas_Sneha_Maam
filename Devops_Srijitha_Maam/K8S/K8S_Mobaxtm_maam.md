...
### Step 1: Disable Swap on All Nodes # MN + WN
...
### Step 2: Enable IPv4 Packet Forwarding # MN + WN

#### sysctl params required by setup, params persist across reboots # MN + WN
```bash
...
#### Apply sysctl params without reboot # MN + WN
```bash
sudo sysctl --system
```

### Step 3: Verify IPv4 Packet Forwarding # MN + WN
```bash
sysctl net.ipv4.ip_forward
```

### Step 4: Install containerd # MN + WN
```bash
...
### Step 5: Install CNI Plugin # MN + WN
```bash
...
### Step 6: Forward IPv4 and Configure iptables # MN + WN # Checking
```bash
...
### Step 7: Modify containerd Configuration for systemd Support do it in every node one by one # MN + WN
```bash
sudo nano /etc/containerd/config.toml
```
####  Delete the content in that file and Paste this configuration in the file and save it. # MN + WN
```bash
disabled_plugins = []
imports = []
oom_score = 0
...
### Step 8: Restart containerd and Check the Status , do for all 3 instance # MN + WN
```bash
sudo systemctl restart containerd && systemctl status containerd
```

### Step 9: Install kubeadm, kubelet, and kubectl , do for all 3 instance # MN + WN
```bash
sudo apt-get update
...
```
### Step 12: Restart kubectl to make the status ready  , do in all nodes
