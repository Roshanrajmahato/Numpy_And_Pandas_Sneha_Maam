...
> `let` and `const` are block-scoped variables introduced in ES6. `let` allow  reassignment but not redeclaration, while `const` does not allow reassignment and must be initialized at declaration. Both are hoisted but exist in a temporal dead zone until initialized.
